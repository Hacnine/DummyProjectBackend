...
export const createClass = async (req, res) => {
  try {
    const { className, classType = "regular" } = req.body
...
export const addModerator = async (req, res) => {
  try {
    ...
    if (!classGroup.group.members.includes(userId)) {
      return res.status(400).json({ message: "User must be a class member first" })
    }
...
export const removeModerator = async (req, res) => {
  try {
    ...
    classGroup.group.moderators = classGroup.group.moderators.filter((mod) => mod.toString() !== userId)
    await classGroup.save()
...
export const removeMember = async (req, res) => {
  try {
    ...
    classGroup.group.members = classGroup.group.members.filter((member) => member.toString() !== userId)
    classGroup.participants = classGroup.participants.filter((participant) => participant.toString() !== userId)
...
export const requestJoinClass = async (req, res) => {
  try {
    ...
    if (!classGroup || !classGroup.group.is_group || classGroup.group.type !== "classroom") {
      return res.status(404).json({ message: "Class not found" })
    }
    ...
    if (existingRequest) {
      ...
      if (existingRequest.status === "approved") {
        return res.status(400).json({ message: "Join request already approved" })
      }
    }
...
export const approveJoinRequest = async (req, res) => {
  try {
    ...
    const joinRequest = await JoinRequest.findOne({ classId, userId, status: "pending" })
    if (!joinRequest) {
